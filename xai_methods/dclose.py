...
class DCLOSE(object):
    def __init__(self, arch, model, img_size=(640, 640), n_segments=150, n_levels=5, n_samples=4000, resize_offset=2.2,
                 batch_size=32, prob=0.5, kernel_width=0.25, seed=0, device='cuda', **kwargs):
        '''
        ...
        for i in range(self.n_levels):
            for idx_obj in range(res.shape[1]):
                if res[4 - i][idx_obj].max() == res[4 - i][idx_obj].min():
                    ...
                    res[4 - i][idx_obj] = (res[4 - i][idx_obj] - res[4 - i][idx_obj].min()) / (
                            res[4 - i][idx_obj].max() - res[4 - i][idx_obj].min())
